...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (allowedTypes.includes(file.mimetype)) {
      ...
      cb(
        new Error(
          "Invalid file type. Only PDF, JPEG, PNG, TIFF, JSON, CSV, and Excel files are allowed.",
        ),
        false,
      );
    }
...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    async (req: any, res) => {
      try {
        ...
        if (!Object.keys(SUBSCRIPTION_PRICES).includes(priceKey)) {
          return res
            .status(400)
            .json({ message: "Invalid subscription tier or billing period" });
        }
        ...
        if (
          ...
          return res
            .status(400)
            .json({ message: `Invalid price key: ${priceKey}` });
        }
  ...
  app.get("/api/lab-results", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: `Failed to fetch lab results: ${error.message}` });
    }
  });
  ...
  app.get("/api/lab-results/:id", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (labResult.userId !== req.user.claims.sub) {
        return res
          .status(403)
          .json({ message: "Unauthorized access to lab result" });
      }
      ...
      res
        .status(500)
        .json({ message: `Failed to fetch lab result: ${error.message}` });
    }
  });
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
          ...
          try {
            ...
            if (
              ...
              await storage.createAiInsight({
                ...
                severity:
                  analysisData.status === "abnormal" ? "alert" : "warning",
              });
            }
          } catch (err) {
        ...
        res
          .status(500)
          .json({ message: `Failed to upload lab result: ${error.message}` });
      }
    },
  );
  ...
  app.delete("/api/lab-results/:id", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: `Failed to delete lab result: ${error.message}` });
    }
  });
  ...
  app.get("/api/bloodwork-markers", isAuthenticated, async (req: any, res) => {
    try {
      ...
      let markers;
      if (name) {
        markers = await storage.getBloodworkMarkersByName(
          userId,
          name as string,
        );
      } else {
      ...
      res.status(500).json({
        message: `Failed to fetch bloodwork markers: ${error.message}`,
      });
    }
  });

  app.get(
    "/api/lab-results/:id/bloodwork-markers",
    isAuthenticated,
    async (req: any, res) => {
      try {
        ...
        if (labResult.userId !== req.user.claims.sub) {
          return res
            .status(403)
            .json({ message: "Unauthorized access to lab result data" });
        }

        const markers =
          await storage.getBloodworkMarkersByLabResult(labResultId);
        res.json(markers);
      } catch (error: any) {
        console.error(
          "Error fetching bloodwork markers for lab result:",
          error,
        );
        res.status(500).json({
          message: `Failed to fetch bloodwork markers: ${error.message}`,
  ...
  // Process a specific lab result
  app.post(
    "/api/lab-results/:id/process",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user.claims.sub;
        ...
        if (category.toLowerCase() === "dexa") {
          ...
          try {
            ...
            // Mark the lab result as processed and completed
            await storage.updateLabResult(labResult.id, {
              status: "completed",
              data: { 
                category: "bloodwork",
            ...
            // Update processed status separately
            await storage.updateLabResultProcessed(labResult.id, true);
            
            // Create an insight
            await storage.createAiInsight({
              userId,
              content: `Your bloodwork "${labResult.title}" has been processed with ${storedMarkers.length} markers extracted.`,
              category: "lab_results",
              severity: "info"
            });
            ...
            return res.json({ 
              ...
              processed: true,
              type: "bloodwork",
              markersCount: storedMarkers.length 
            });
          } catch (error: any) {
        ...
        res.status(500).json({ 
          message: `Failed to process lab result: ${error.message}` 
        });
      }
    }
  );

  // Background task to process all lab results and extract bloodwork markers
  app.post(
    "/api/lab-results/process",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user.claims.sub;
        ...
        console.log(
          `Found ${labResults.length} lab results for user ${userId}`,
        );
        const unprocessedResults = labResults.filter(
          (result) =>
        ...
        if (unprocessedResults.length === 0) {
          return res.json({
            message: "No unprocessed lab results found",
            count: 0,
          });
        }
        ...
        const processPromises = unprocessedResults.map(async (labResult) => {
          try {
            console.log(
              `Processing lab result ${labResult.id} from ${labResult.fileUrl}`,
            );
            ...
            console.log(
              `Processing lab result ${labResult.id} with category: ${category}`,
            );

            // Extract date
            const resultDate =
              labResult.resultDate ||
              new Date(labResult.uploadedAt || new Date())
                .toISOString()
                .split("T")[0];
            ...
            if (category.toLowerCase() === "dexa") {
              try {
                ...
                console.error(
                  `Error processing DEXA scan ${labResult.id}:`,
                  error,
                );
                await storage.updateLabResult(labResult.id, {
                  status: "error",
              ...
              try {
                ...
                if (markers && markers.length > 0) {
                  // Store the extracted markers
                  storedMarkers =
                    await storage.batchCreateBloodworkMarkers(markers);
                  console.log(
                    `Created ${storedMarkers.length} bloodwork markers for lab result ${labResult.id}`,
                  );

                  // Create an insight if any abnormal values were found
                  const abnormalMarkers = storedMarkers.filter(
                    (marker) => marker.isAbnormal,
                  );
                  if (abnormalMarkers.length > 0) {
                    const markerNames = abnormalMarkers
                      .map((m) => m.name)
                      .join(", ");
                    await storage.createAiInsight({
                      userId,
                ...
                console.error(
                  `Error processing bloodwork ${labResult.id}:`,
                  error,
                );
                await storage.updateLabResult(labResult.id, {
                  status: "error",
            ...
            console.error(
              `Error processing lab result ${labResult.id}:`,
              error,
            );
            return {
              labResultId: labResult.id,
        ...
        res
          .status(500)
          .json({ message: `Failed to process lab results: ${error.message}` });
      }
    },
  );
  ...
  app.get("/api/health-metrics", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: `Failed to fetch health metrics: ${error.message}` });
    }
  });
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        res.status(500).json({
          message: `Failed to fetch latest health metric: ${error.message}`,
        });
      }
    },
  );
  ...
  app.post("/api/health-metrics", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: `Failed to create health metric: ${error.message}` });
    }
  });
    ...
    async (req: any, res) => {
      try {
        ...
        if (!Array.isArray(metrics)) {
          return res
            .status(400)
            .json({ message: "Invalid metrics data. Expected an array." });
        }
        ...
        const createdMetrics =
          await storage.batchCreateHealthMetrics(metricsWithUserId);
        res.json(createdMetrics);
      } catch (error: any) {
        console.error("Error creating batch health metrics:", error);
        res.status(500).json({
          message: `Failed to create batch health metrics: ${error.message}`,
        });
      }
    },
  );
  ...
  app.get("/api/ai-insights", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: `Failed to fetch AI insights: ${error.message}` });
    }
  });
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        res.status(500).json({
          message: `Failed to mark insight as read: ${error.message}`,
        });
      }
    },
  );
  ...
  app.get("/api/health-events", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: `Failed to fetch health events: ${error.message}` });
    }
  });
  ...
  app.post("/api/health-events", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: `Failed to create health event: ${error.message}` });
    }
  });
  ...
  app.put("/api/health-events/:id", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: `Failed to update health event: ${error.message}` });
    }
  });
  ...
  app.delete(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: `Failed to delete health event: ${error.message}` });
      }
    },
  );
  ...
  app.get("/api/connected-services", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res.status(500).json({
        message: `Failed to fetch connected services: ${error.message}`,
      });
    }
  });

  app.post(
    "/api/connected-services/:serviceName/connect",
    isAuthenticated,
    async (req: any, res) => {
      ...
      try {
        ...
        res.status(500).json({
          message: `Failed to connect ${serviceName}: ${error.message}`,
        });
      }
    },
  );
    ...
    async (req: any, res) => {
      try {
        ...
        if (!data) {
          return res
            .status(400)
            .json({ message: "No Apple Health data provided" });
        }

        // Import the necessary functions from the apple-health module
        const { validateAppleHealthData, processAppleHealthData } =
          await import("./apple-health");
        ...
        res.status(500).json({
          message: `Failed to sync Apple Health data: ${error.message}`,
        });
      }
    },
  );
  ...
  app.post(
    "/api/connected-services/:serviceName/disconnect",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user.claims.sub;
        ...
        res.status(500).json({
          message: `Failed to disconnect ${req.params.serviceName}: ${error.message}`,
        });
      }
    },
  );
  ...
  app.get("/api/health-plan", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: `Failed to generate health plan: ${error.message}` });
    }
  });
  ...
  app.get("/api/workouts", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: `Failed to fetch workouts: ${error.message}` });
    }
  });
  ...
  app.get("/api/workouts/:id", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: `Failed to fetch workout: ${error.message}` });
    }
  });
  ...
  app.post("/api/workouts", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: `Failed to create workout: ${error.message}` });
    }
  });
  ...
  app.put("/api/workouts/:id", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: `Failed to update workout: ${error.message}` });
    }
  });
  ...
  app.delete("/api/workouts/:id", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: `Failed to delete workout: ${error.message}` });
    }
  });
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: `Failed to fetch workout sets: ${error.message}` });
      }
    },
  );
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: `Failed to create workout set: ${error.message}` });
      }
    },
  );
  ...
  app.put("/api/workout-sets/:id", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: `Failed to update workout set: ${error.message}` });
    }
  });
  ...
  app.delete(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: `Failed to delete workout set: ${error.message}` });
      }
    },
  );
